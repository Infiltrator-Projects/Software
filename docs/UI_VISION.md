...
The Discover hero uses a dedicated authored raster only as scenery. It contains no product wording, no red-and-white checker sphere, no Amiga or other recognisable retro-computer prop, and no platform mascot. Every visible word in the hero is live GTK text layered above the raster so it scales, localises and remains accessible. The approved visual language is atmospheric blue/purple sky, mountains, light and abstract landscape depth.

The application title bar is product UI, not a generic desktop toolbar. Its identity mark is an authored raster containing two angled parallel rainbow slashes. It must never be reconstructed as three procedural coloured GtkBox bars. The raster mark and product identity sit at the left, a broad centred search field carries its shortcut cue, then product actions, a separator and compact native window controls occupy the far right. In dark mode these controls remain dark navy/blue with light glyphs; they must never regress to oversized pale system buttons.

The Discover composition is also structural, not decorative. Immediately below the hero are exactly three dashboard cards: Available Updates, System Health and Repositories. Beneath those, Featured Applications owns roughly two thirds of the main dashboard width and is populated immediately with four useful install surfaces (Firefox, LibreOffice, GIMP and VLC), reconciling them with exact catalogue/install state when that state becomes available. Repository Status and Recent Activity form the narrower right rail. A catalogue refresh must never leave the Featured Applications panel as an empty decorative box. The shell itself is responsive: navigation owns one sixth of the body and the content stack five sixths, while the main Discover dashboard uses a homogeneous three-column grid with Featured/Updates/Health spanning two columns and Repository/Activity one.
