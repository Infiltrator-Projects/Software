...
The shared service is implemented as a session D-Bus service at `net.ssmith.infiltrator.software.Engine`. It owns the in-memory view of the latest package-state generation, caches the update calculation once per generation, exposes status/installed/update snapshots and native transaction planning, and emits generation/health changes. It is D-Bus activatable and keeps stale known-good state readable if a subsequent database reload fails. Installed inventory and ordinary Updates prefer this shared engine when a published generation is available, the tray subscribes to its state and health signals instead of running a second resolver, and native reconciliation refreshes configured repository sources plus installed dpkg state before atomically publishing a complete generation. Remaining APT use is confined to the transitional privileged execution boundary and compatibility fallback.
