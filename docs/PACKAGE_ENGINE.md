...
The eighth native slice implements deterministic transaction planning for install, upgrade and removal operations. Install/upgrade plans are tied to a state generation and source fingerprint, select requested roots using repository priority and Debian version rules, expand dependencies through the native resolver, reject unresolved Conflicts/Breaks, honour holds and downgrade policy, distinguish requested from dependency-induced changes, carry repository/payload provenance and SHA-256 into each item, and calculate total download bytes plus signed installed-size delta. Removal plans retain installed Depends, Pre-Depends, Provides, Priority, Multi-Arch and Essential metadata in the generation, build the retained package set, prove every retained hard dependency remains satisfied, reject Essential removal, and calculate the exact negative disk-size delta before authorization.
