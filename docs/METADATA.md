...
The pre-install catalogue icon and the desktop icon installed by the package originate from the same release asset and are tied by digest. Software may cache an icon by digest, but the cache never becomes a competing source of identity.
